...
| 개발 deployment | `dev:abundant-retriever-951` (이미 함수 배포·동작 확인 완료) |
| 운영 deployment | **아직 없음** — 1단계에서 만들어진다 |
| 레포 | `C:\Users\유범석\개발소스코드\26_03\암요양app\oncocare-bug-report` (그룹웨어와 **별도**) |
...
1. https://vercel.com → **Add New → Project** → 이 레포 import
   (아직 GitHub 에 안 올렸다면 먼저 push. 그룹웨어 레포와 **다른 레포**다)
2. **Root Directory**: `oncocare-bug-report` (레포 루트가 이미 여기라면 그대로 둔다)
3. **Build Command** 를 아래로 **덮어쓴다** (Override 켜기):

```
npx convex deploy --cmd-url-env-var-name CONVEX_URL --cmd 'npm run build'
```

> 이러면 Vercel 이 빌드할 때 ① Convex 함수를 운영에 배포하고 ② `CONVEX_URL` 환경변수를 운영 URL 로 자동 주입한 뒤
> ③ `next build` 를 돌린다. `CONVEX_URL` 을 손으로 넣을 필요가 없다.

4. **Environment Variables** (Production·Preview 모두):
